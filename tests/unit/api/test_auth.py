...
class TestAuthService:
    ...
    def test_login_rejects_inactive_user(self) -> None:
        # INFRA-008 (found in live smoke): a deactivated user must not be
        # issued fresh tokens at all, not merely rejected on the next request.
        self.user_repo.set_active("user-1", False)
        with pytest.raises(AuthenticationError) as exc_info:
            self.service.login("test@example.com", "correct-password")
