...
**Next action:** AUTHOR-012 done (PR #88 → develop `ab354f7`, merged 2026-09-01; the credit-blocked smoke re-run passed the same day — see PROGRESS.md RESUME block). Next is the user's pick: PUBLISH-002 (WordPress, 5 SP), AUTHOR-011 (persona voice engine, 13 SP), or AUTHOR-013 (LinkedIn repurpose, 5 SP). Environment clean: no worktrees, Anthropic credits topped up, Docker api+worker rebuilt on `ab354f7` and verified (`GET /prompts` = 28 keys).
