...
| Visuals — cap at 3 per article + interleave fix (PR #66) | In Progress | 2026-05-25 | Dashboard preview was stacking every planner-rendered visual at the top (25 of them on a 7-section article). Frontend bucketing read legacy `metadata.source_section` while the planner stamps `metadata.section_index` — every asset fell into the cover bucket. Plus no per-article image cap. `image_planner_max_images_per_section` 4→1, new `image_planner_max_total_images=3` enforced post-planning, per-section heroes dropped (only article cover may be hero). Branch `fix/visuals-cap-and-interleave`, commit `6e59711`. PR open against develop; local containers still on stale image (Docker Desktop hung mid-rebuild — verification pending). |
