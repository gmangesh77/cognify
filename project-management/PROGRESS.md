...
| AUTHOR-005 | Usage/cost badge | In Progress (2026-08-24) | `feature/AUTHOR-005-usage` | [plan](../docs/superpowers/plans/2026-08-24-author-005-usage.md) | program plan §5.6; review §6 #5 |
...
> 4. **AUTHOR-005 (usage/cost badge — program plan §5.6, review §6 #5) — IN PROGRESS**: worktree `feature/AUTHOR-005-usage` off `develop` (`.claude/worktrees/author-005-usage`). `llm_calls` rows for regenerate are already keyed by the real `draft.session_id` with `call_name=section_regenerate`, so the usage endpoint can aggregate them directly. Azure Boards has no Epic 11 work items (checked 2026-08-21) — nothing to set Active.
>
