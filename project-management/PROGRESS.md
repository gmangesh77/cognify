...
| AUTHOR-005 | Usage/cost badge | **Next** (Planned) | — | program plan §5.6 | review §6 #5 |
...
> **RESUME HERE (2026-08-21 EOD — start of next session):** `develop` is at `7ef590c` (PR #76 AUTHOR-004 merged). All worktrees removed; only the main checkout remains. **No code work was started after #76.** First actions tomorrow, in order:
> 1. `docker compose up --build -d` — the running stack is still on the **pre-#76** images (api built before the merge, frontend ~6 h old). Per the known gotcha, verify the frontend image timestamp advanced; if the Next chunk hash did not change, rebuild the frontend with `--no-cache`. No Alembic migration in #76 (DB stays at `c4d8e2f1a9b7`).
> 2. **Live smoke of AUTHOR-004 + L-013** (never run yet): open an article → hover the **second** section → Edit text must show the second section's markdown (before #76 it showed the first) → Cancel → hover a section → Regenerate → optional instruction → diff + "N words · model" → Accept → section re-renders, figure anchors intact, neighbours untouched → History drawer shows "Regenerated" twice (candidate + applied) → Reject path changes nothing → `/pipeline-debug` for that article's session shows one `section_regenerate` LLM call. Note: Chrome-extension clicks did not register on this app last time — drive with `javascript_tool` `.click()` / native value setters if needed.
> 3. **L-013 post-deploy audit**: run the three SQL queries in `docs/LEARNINGS.md` → L-013 against the local DB once and record the result here (pre-fix AI-rewrite rows / duplicated-H2 bodies / Studio visual indices).
> 4. Then start **AUTHOR-005** (usage/cost badge — program plan §5.6, review §6 #5): worktree `feature/AUTHOR-005-usage` off `develop`, mark In Progress here, write its TDD plan. `llm_calls` rows for regenerate are already keyed by the real `draft.session_id` with `step_name=section_regenerate`, so the usage endpoint can aggregate them directly. Azure Boards has no Epic 11 work items (checked 2026-08-21) — nothing to set Active.
>
> Follow-up tickets still to open (not started): `Provenance.research_session_id` is the topic id at the source (`graph_state.py` → `seo_node.py`; AUTHOR-001's `articles.find_by_session` relies on it — fix both together); humanize pass on regenerated prose (AUTHOR-009); `content.py` / `section_rewriter.py` / `main.py` / `services/content/__init__.py` / `db/repositories.py` >200 lines (INFRA-008); `RegeneratePopover` has no Escape/focus-trap; `hasPreamble()` vs backend `split_sections` on `###`-first bodies; Studio section list includes the References tail; `word_count` measured on raw LLM text; `/health` reports redis/milvus/celery `unavailable` while the app works; 13 pre-existing `tsc` errors in untouched settings/test files; AUTHOR-003 leftovers (PATCH /briefs can't clear nullable fields, `audience_persona` missing from `ResearchSessionResponse`).
>
